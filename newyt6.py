...
#cursor.execute("create database ytdata3")
#ytdb.commit()
...
def data_query(chosen_query):
    ...
    if chosen_query == 'Names of all the videos and their corresponding channels':
        cursor.execute('select v.Video_Title, c.Channel_Name from yvideo2 v join yplaylist2 p on v.Playlist_ID = p.Playlist_ID join ychannel2 c on p.Channel_ID = c.Channel_ID')
        ...
        query = 'select Channel_name, Video_Count from ychannel2 ORDER BY Video_Count DESC LIMIT 3'
        query_df = pd.read_sql_query(query, ytdb)

    elif chosen_query == 'Top 10 most viewed videos with their channel name':
        query = 'select v.Video_Title, c.Channel_name from yvideo2 v join yplaylist2 p on v.Playlist_ID = p.Playlist_ID join ychannel2 c on p.Channel_ID = c.Channel_ID ORDER BY v.View_Count DESC LIMIT 10'
        query_df = pd.read_sql_query(query, ytdb)

    elif chosen_query == 'Number of comments on each video with channel name':
        query = 'select v.Video_Title, v.Comment_Count, c.Channel_Name from yvideo2 v join yplaylist2 p on v.Playlist_ID = p.Playlist_ID join ychannel2 c on p.Channel_ID = c.Channel_ID'
        query_df = pd.read_sql_query(query, ytdb)

    elif chosen_query == 'Videos with highest number of likes with channel name':
        query = 'select v.Video_Title, v.Like_Count, c.Channel_name from yvideo2 v join yplaylist2 p on v.Playlist_ID = p.Playlist_ID join ychannel2 c on p.Channel_ID = c.Channel_ID  ORDER BY v.Like_Count DESC LIMIT 10'

        query_df = pd.read_sql_query(query, ytdb)

    elif chosen_query == 'Number of likes and dislikes of each video':
        query = 'select yvideo2.Video_Title, yvideo2.Like_Count, yvideo2.Dislike_Count from yvideo2'
        query_df = pd.read_sql_query(query, ytdb)

    elif chosen_query == 'Total views of each channel':
        query = 'select ychannel2.Channel_Name, ychannel2.View_Count from ychannel2'
        query_df = pd.read_sql_query(query, ytdb)

    elif chosen_query == 'Names of all the channels that have published videos in the year 2022':
        query = 'select distinct c.Channel_Name from ychannel2 c  join yplaylist2 p on p.Channel_ID = c.Channel_ID join yvideo2 v on v.Playlist_ID = p.Playlist_ID where year(v.Published_At)=2022'
        query_df = pd.read_sql_query(query, ytdb)

    elif chosen_query == 'Average duration of videos in each channel':
        query = 'select c.Channel_Name, AVG(v.Duration) as avg_duration_seconds from ychannel2 c  join yplaylist2 p on p.Channel_ID = c.Channel_ID join yvideo2 v on v.Playlist_ID = p.Playlist_ID GROUP BY c.Channel_Name'
        query_df = pd.read_sql_query(query, ytdb)

    elif chosen_query == 'Videos with most comments with channel name':
        query = 'select v.Video_Title, v.Comment_Count, c.Channel_Name from yvideo2 v  join yplaylist2 p on v.Playlist_ID = p.Playlist_ID  join ychannel2 c on p.Channel_ID = c.Channel_ID  ORDER BY v.Comment_Count DESC LIMIT 10'
        query_df = pd.read_sql_query(query, ytdb)
